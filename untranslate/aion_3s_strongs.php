...
$database['MISS_VARIANT']	= "Variant manuscript abbreviations in tagged texts, but undefined\n===\n\n\n";
...
//AION_NEWSTRONGS_COUNT_REF_CHECKER("$FOLDER_DEBUG$HEBREW_TAGED_NUMS", "$INPUT_TOTH1", "$INPUT_TOTH2", "$INPUT_TOTH3", "$INPUT_TOTH4");
...
AION_NEWSTRONGS_GET( "$INPUT_TAGNT",'41_Mat.001.001	002', NULL, 'GREREB',array('REF','WNUM','GREEK','ENGLISH','STRONGS','MORPH','DICT','GLOSS','EDITIONS','VARSPELL','VARMEAN','SPANISH','SUB','SUPER','COJOIN'), NULL, NULL, $database);
// TYNDALE GREEK WRITE
...
if ( file_put_contents($file=$CHECK_MISS, $database['MISS_VARIANT']) === FALSE) {			AION_ECHO("ERROR! file_put: ".$file ); }
AION_ECHO("CHECK $CHECK_MISS ROWS=".substr_count($database['MISS_VARIANT'], "\n"));
...
function AION_NEWSTRONGS_GET_FIX($file, $contents, &$result) {
	...
	$contents = preg_replace($reg="#[ ]+[\r\n]+#usi",							"\n",	$contents,-1,$count);		if ($count) { $result['FIXCOUNTS'].="$newmess\t\ttrim=$count\n"; }
	// bye bye
	return($contents);
}
...
function AION_NEWSTRONGS_GET_FIX_LEX($file, &$lines, &$database, $morph_array) {
	...
	foreach( $lines as $x => $line ) {
		...
		$report='';
		if (preg_match_all("#.{0,30}<[^>]+>.{0,30}#ui",$lines[$x]['DEF'],$match,PREG_PATTERN_ORDER)) {
		...
		else if (preg_match_all("#.{0,30}<.{0,30}#ui",$lines[$x]['DEF'],$match,PREG_PATTERN_ORDER)) {
			foreach($match[0] as $x => $suspect) { $n=$x+1; $report .= "$n) '$suspect'\n"; }
...
function AION_NEWSTRONGS_FIX_REF($input,$what,$table,&$database, &$lex_array, &$lex2_array, $morph_array) {
	...
	foreach( $input as $line ) {
		...
		$newmess = "FIX_REF\t$what\tref='".$line['REF']."'\tword='$WORDUP'\tmorph='".$line['MORPH']."'\tstrongs='".$line['STRONGS']."'";

		// PARSE REFERENCE
		if ($what=='Hebrew') { if (!preg_match("#^([0-9A-Za-z]+)[[:punct:]]+([\d]+)[[:punct:]]+([\d]+)[[:punct:]]+([\d]+)([kq]{0,1})$#u",$line['REF'],$match)) {	AION_ECHO("ERROR! $newmess corrupt hebrew ref\n".print_r($line,TRUE)); } }
		else { if (!preg_match("#^[\d]+[[:punct:]]+([0-9A-Za-z]+)[[:punct:]]+([\d]+)[[:punct:]]+([\d]+)$#u",$line['REF'],$match)) {	AION_ECHO("ERROR! $newmess corrupt greek ref\n".print_r($line,TRUE)); } }
		...
		// DUPLICATES?
		if (($what=='Hebrew' && !empty($line_last['REF']) && $line_last['REF']==$line['REF']) ||
		...
		$plus = (($what=='Greek' && preg_match("#\+#ui",$line['STRONGS'])) ? TRUE : FALSE); // if Greek + then second strongs shares the morph
		if ($what=='Greek') { $spart = mb_split("\+", $line['STRONGS']); }
		else { $spart = mb_split("/", $line['STRONGS']); }
		...
		foreach($spart as $key => $part) {
			...
			// MORPHS
			$morph = ($key==0 ? $mpart[0] : ($plus && empty($mpart[$key]) ? $mpart[0] : (empty($mpart[$key]) ? '' : $letter.$mpart[$key])));
			...
			if ($what=='Greek') {
				...
				if (!empty($line['VARIANTS'])) {
					$vparts = mb_split(";", trim($line['VARIANTS']));
					
					// LOOP THRU VARIANT PARTS
					foreach($vparts as $vpart) {
						if (empty($vpart)) { continue; }
						$vpart = trim($vpart);
						$newmess = "FIX_REF\t$what\tref='".$line['REF']."'\tword='$WORDUP'\tmorph='".$line['MORPH']."'\tstrongs='".$line['STRONGS']."'\tvariants='".$line['VARIANTS']."'\tvpart='$vpart'";
						//TH=κοπιοῦσιν=G2872=V-PAI-3P (V-PAP-DPM);
						//IMNSTW=>;
						//IMNSTW=<;
						if (preg_match("#^([^=<>]+)=([<>]{1})$#ui",$vpart,$match)) {
						...
						else if (preg_match("#^([^=()<>]+)=([^=()<>]+)=([^=()<>]+)=([^=()<>]+)(.*)$#ui",$vpart,$match)) {
							$match[1] = trim($match[1]); // source abbreviations
							$match[2] = trim($match[2]); // Greek word
							$match[3] = AION_NEWSTRONGS_STRONGS_PARSE($newmess, $match[3], TRUE, $lex_array, $lex2_array); // parse Strongs
							$match[4] = trim($match[4]); // morphhology
							if (empty($morph_array[$match[4]])) {
								$database['MISS_MORPHS'] .= ($warn="$newmess\tmissing morph='$match[4]'\n");
								AION_ECHO("WARN!\t$warn".print_r($line,TRUE)."\n\n\n");
							}
							$match[5] = (empty($match[5]) ? "" : trim($match[5]," ()")); // second morphhology
							if (!empty($match[5]) && empty($morph_array[$match[5]])) {
								$database['MISS_MORPHS'] .= ($warn="$newmess\tmissing morph2='$match[5]'\n");
								AION_ECHO("WARN!\t$warn".print_r($line,TRUE)."\n\n\n");
							}
							$variants .= "/$match[1]=$match[2]=$match[3]";
						}
						else {
							$database['CORRUPT_VARIANT'] .= ($warn="$newmess\tcorrupt vpart\n");
							AION_ECHO("WARN!\t$warn".print_r($line,TRUE)."\n\n\n");	
							continue;
						}
						// source abbreviations ok?
						$vararray = str_split($match[1]);
						foreach($vararray as $code) {
							if(!empty($vartrans[$code])) { continue; }
							$database['MISS_VARIANT'] .= ($warn="$newmess\tmissing code='$code'\n");
							AION_ECHO("WARN!\t$warn".print_r($line,TRUE)."\n\n\n");
						}
					}
				}
			...
			$database[$table] .= "$indx\t$book\t$chap\t$vers\t$strongs\t$jointype\t$morph\t$WORDUP$variants\n";
			// W=next word, Ketiv=written word, Qere=read word, P=word parts, J=joined words, D=divided word
			if ($what=='Hebrew') {	$jointype = "P"; }
			else {					$jointype = "J"; }
...
function AION_NEWSTRONGS_STRONGS_PARSE($newmess, $strongs, $variant, &$lex_array, &$lex2_array) {
	...
	foreach($parsed as $x => $set) {
		...
		if (!empty($lex_array)) {
			...
			if ($strong != $naked_strong) {
				if (empty($lex_array[$naked_strong])) { AION_ECHO("ERROR! $newmess PARSE(strongs=$strongs, strong=$naked_strong) not in lexicon"); } // not found
				else { $lex_array[$naked_strong]['WORD'] = $lex_array[$naked_strong]['TRANS'] = $lex_array[$naked_strong]['MORPH'] = $lex_array[$naked_strong]['DEF'] = NULL; } // found
			}
		}
		if (!empty($lex2_array)) {
			...
			if ($strong != $naked_strong) {
				if (empty($lex2_array[$naked_strong])) { AION_ECHO("ERROR! $newmess PARSE(strongs=$strongs, strong=$naked_strong) not in lexicon"); } // not found
				else { $lex2_array[$naked_strong]['WORD'] = $lex2_array[$naked_strong]['TRANS'] = $lex2_array[$naked_strong]['MORPH'] = $lex2_array[$naked_strong]['DEF'] = NULL; } // found
			}
		}
		// variant only?
		if ($variant) {
			...
			if ($strong != $naked_strong) {
				if (!empty($lex_array[$naked_strong])) {	$lex_array[$naked_strong]['VARIANT'] = "Variant usage ONLY"; }
				if (!empty($lex2_array[$naked_strong])) {	$lex2_array[$naked_strong]['VARIANT'] = "Variant usage ONLY"; }
			}
		}
		// build return
...
function AION_NEWSTRONGS_COUNT_REF($references, $output) {
	...
	while ($line !== false) {
		...
		// counts
		if ($indx_last != NULL) {
			if (($yes=($indx != $indx_last))) {				foreach($book_array as $s => $x) {	++$yeah_array[$s][0]; }			unset($book_array);	$book_array = array(); }
			if ($yes || (($yes=($chap != $chap_last)))) {	foreach($chap_array as $s => $x) {	++$yeah_array[$s][1]; }			unset($chap_array);	$chap_array = array(); }
			if ($yes || ($vers != $vers_last)) {			foreach($vers_array as $s => $x) {	if (empty($yeah_array[$s])) {	$yeah_array[$s] = array(0,0,0,0); }
																								++$yeah_array[$s][2];
																								$yeah_array[$s][3] += $x; }		unset($vers_array);	$vers_array = array(); }	
		}
		// wrap up and next
		$indx_last = $indx;
		$chap_last = $chap;
...
function AION_NEWSTRONGS_COUNT_REF_CHECKER($countsF, $source1, $source2, $source3, $source4) {
	...
	if (($source  = file_get_contents( $source1 )) === FALSE ) {				AION_ECHO("ERROR! $newmess !file_get_contents(1)"); }
	if ($source2 && ($sourceT = file_get_contents( $source2 )) === FALSE ) {	AION_ECHO("ERROR! $newmess !file_get_contents(1)"); }	$source .= $sourceT;
	if ($source3 && ($sourceT = file_get_contents( $source3 )) === FALSE ) {	AION_ECHO("ERROR! $newmess !file_get_contents(1)"); }	$source .= $sourceT;
	if ($source4 && ($sourceT = file_get_contents( $source4 )) === FALSE ) {	AION_ECHO("ERROR! $newmess !file_get_contents(1)"); }	$source .= $sourceT;
	unset($sourceT);
	// loop through counts
	foreach($counts as $strongs => $numbers) {
		...
		if (FALSE===preg_match_all("#($strongs)[^\d]{1}#u", $source, $parsed, PREG_PATTERN_ORDER)) { AION_ECHO("ERROR! $newmess !preg_match_all()"); }
		if ($numbers[3] != ($found=count($parsed[1]))) {
			AION_ECHO("WARN! $newmess count mismatch for $strongs: $numbers[3] != $found");
		}
	}
	return;
}
...
function AION_NEWSTRONGS_USAGE_FOLDER($tagf, $lex1, $lex2, $lex3, $fold) {
	...
	while ($line !== false) {
		$tagA = explode("\t",$line);
		if ($tagA[0][0]=="#" || $tagA[0]=="STRONGS" || count($tagA)<8) { $line = strtok( "\n" ); continue; }
		$tag = array(
			"INDEX"		=> $tagA[0],
			"BOOK"		=> $tagA[1],
			"CHAPTER"	=> $tagA[2],
		...
		if (!empty($tagA[8])) { $tag["VARIANT"] = $tagA[8]; }
		// write the json file
		...
		if (!empty($tag["VARIANT"])) {	$temp["V"] = $tag["VARIANT"]; }
		if (!empty($database['T_LEX1'][$strongs])) {	$temp["1"] = AION_NEWSTRONGS_ASSOCIATIVE_RENAME($database['T_LEX1'][$strongs]); }
